...
import { WallType, Wall } from '../models/Wall';
...
export class GameController {
    ...
    compassView: MagicCompassView;
    narrator: Narrator;
    selectedPlayer: Player | null;
    gameState: GameState;
    debugViewAll: boolean;
    debugEditMode: boolean;

    constructor(game: Game, boardView: BoardView, selectionView: SelectionView, compassView: MagicCompassView, narrator: Narrator) {
        this.game = game;
    ...
    async handleSelectionView(data: { players: Player[], firstPlayer: Player | null }) {
    ...
    async handleTileClick(tile: Tile) {
        ...
        try {
            direction = Direction.getDirection(tile.row - player.row, tile.col - player.col);
        } catch (e) {
            console.error(e);
            this.narrator.invalidMove();
            return;
        }

        this.compassView.togglePlayerLightOff(player);
    ...
    handleWallRightClick(wall: Wall) {
        this.selectedPlayer = null;
        if (wall.isExternal) return;
    ...
    async _handlePlayerClick(playerInfo: Player) {
    ...
    async _handleSelectPlayer(playerInfo: Player) {
        if (await this.game.addPlayer(playerInfo)) {
            this.compassView.togglePlayerLightOn(playerInfo);
        }
    }

    async _handleSelectFirstPlayer(playerInfo: Player) {
    ...
    async _handleDirectionClick(direction: Direction) {
        if (this._onInterruption()) {
            return;
        }
