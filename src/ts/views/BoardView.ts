...
export class BoardView {
    ...
    wallElements: Map<string, HTMLElement>;
    navClickCount: number;
    messageBox!: HTMLElement;
    debugViewAll: boolean = false;
    debugEditMode: boolean = false;
    ...
    render(game: Game, controller: GameController) {
    ...
    bindToggleDebugView(handler: () => void) {
        this.onToggleDebugView = handler;
    }

    bindToggleEditMode(handler: () => void) {
        this.onToggleEditMode = handler;
    }

    bindNavigateToCompass(handler: () => void) {
        this.onNavigateToCompass = handler;
    }

    bindGoBack(handler: () => void) {
        this.onGoBack = handler;
    }

    setDebugState(viewAll: boolean, editMode: boolean) {
    ...
    createTileElement(tile: Tile, game: Game, controller: GameController, container: HTMLElement) {
        const div = document.createElement('div');
        div.className = 'tile';
        div.dataset.row = tile.row.toString();
        div.dataset.col = tile.col.toString();
        ...
        if (game.players) {
            const players = Array.from(game.players.values()).filter(p => p.row === tile.row && p.col === tile.col);
