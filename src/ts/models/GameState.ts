...
export class GameState {
    ...
    narrator: Narrator;
    currentState: string;
    requiredMagicDoorCount: number;
    openedMagicDoorCount: number;
    ...
    async tryOpenDoor(door: Wall) {
        ...
        if (this.game.getPlayersAt(tiles[0].row, tiles[0].col).length >= 1
            ...
            if (!this.game.currentPlayer) return false;
    ...
    async checkGameOverCondition() {
        ...
        if (this.game.totalTurns == Math.floor(midnightMoves * Constants.SUNSET_PERCENTAGE)) {
            ...
            await this.narrator.midnight(); 4
    ...
    async _validatePlayers() {
        ...
        const rabbits = players.values().filter((p: Player) => p.type === PlayerType.RABBIT).toArray();
        const mice = players.values().filter((p: Player) => p.type === PlayerType.MOUSE).toArray();
    ...
    _computeRequiredMagicDoorCount(difficulty: Difficulty): number {
