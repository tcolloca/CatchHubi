...
import { GhostHint } from './models/GhostHint';
import { DoorHint } from './models/DoorHint';
...
export class Narrator {
    ...
    messages: IMessages;
    language: Language;
    _waitingForMessage: number;
    _recordingTurn: boolean;
    _lastTurnHistory: any[];
    _currentTurnHistory: any[];
    _messagePromise: Promise<void> | null;
    _resolveInterruption: (() => void) | null;
    _messageTimeout: number | null;
    _interrupted: boolean;

    constructor(audioManager: AudioManager, messages: IMessages, language: Language) {
    ...
    async addPlayer(playerInfo: Player) {
        await this._playSequence('host', [
    ...
    async playerAlreadyInGame(playerInfo: Player) {
        await this._playSequence('host', [
            this._playerName(playerInfo),
    ...
    async firstPlayerSelected(playerInfo: Player) {
        await this._playSequence('host', [
            this._playerName(playerInfo),
    ...
    async askWhereTo(creature: Creature, playerInfo: Player) {
    ...
    async magicDoorOpened(creature: Creature, wakeUpHubi: boolean) {
    ...
    async announceWall(creature: Creature, wall: Wall) {
    ...
    async canPassThrough(creature: Creature) {
        await this._playSequence(creature.type, [
            'can_pass_through.m4a',
        ], 'can_pass_through', []);
    }

    async cannotPassThrough(creature: Creature) {
        await this._playSequence(creature.type, [
            'cannot_pass_through.m4a',
        ], 'cannot_pass_through', []);
    }

    async giveHint(creature: Creature, hint: GhostHint | DoorHint) {
    ...
    async givePlayerPosition(playerInfo: Player, creature: Creature, owl: Creature, color: string) {
        await this._playSequence('host', [
            this._playerName(playerInfo),
            'player_position.m4a',
    ...
    async bonusMove(creature: Creature) {
        await this._playSequence(creature.type, [
            'bonus_move.m4a',
        ], 'bonus_move');
    }

    async itsPlayerTurn(playerInfo: Player) {
        await this._playSequence('host', [
            this._playerName(playerInfo),
            'its_player_turn.m4a',
        ], 'its_player_turn', [playerInfo.name]);
    }
    ...
    async _playSequence(source: string, filesSequence: string[], labelId: string | null = null, args: string[] = []) {
    ...
    async _displayMessage(source: string, labelId: string, args: string[]) {
    ...
    _getMessage(labelId: string, args: string[] = []) {
